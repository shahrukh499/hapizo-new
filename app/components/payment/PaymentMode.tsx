...
export default function PaymentMode() {
  ...
  const {
    ...
    couponCode,
    appliedCoupon,
  } = useAppSelector((state) => state.couponSlice);
  ...
  const shipping = 0;
  const grandTotal =
    subTotal + serviceCharge + shipping - totalDiscount - (couponDiscount || 0);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      if (selectedValue === "COD") {
        ...
        router.push(
          `/order-success?orderId=${data?.orders[0]?.orderId}&paymentMethod=${data?.orders[0]?.paymentMethod}&street=${data?.orders[0]?.address?.street}&city=${data?.orders[0]?.address?.city}&state=${data?.orders[0]?.address?.state}&zip=${data?.orders[0]?.address?.zip}&amount=${data?.orders[0]?.totalAmount}&deliveryDate=${data?.orders[0]?.createdAt}`,
        );
  ...
  return (
    <div className="pt-4 mb-3">
      <div
        className={`${selectedValue === "COD" ? "border-2 border-purple-400 bg-purple-50 rounded-2xl" : "border-2 border-gray-200 rounded-2xl"} px-1 lg:px-5 py-5 lg:py-10`}
      >
            ...
            <div>
              ...
              <div className="mb-2 bg-green-100 rounded-full inline-flex px-3 py-[2px] gap-x-1 items-center">
                <CreditCardOutlinedIcon
                  ...
                  sx={{ color: "#008236" }}
                />
                <span className="text-[13px] text-center text-green-700 mt-1">
                  Pay when you receive your order
                </span>
              </div>
              <p className="text-[15px] mb-2 w-full max-w-[250px] text-gray-500">
                Pay in cash or UPI to the deliver partner at the time of
                deleivry
              </p>
              <div className="flex items-center gap-x-3">
                <div className="flex items-center justify-center gap-x-1 shadow bg-white px-3 rounded-full">
                  <Image
                    src="/assets/img/cash1.png"
                    alt="cash"
                    width={25}
                    height={25}
                  />
      ...
      <div
        className={`${selectedValue === "ONLINE" ? "border-2 border-purple-400 bg-purple-50 rounded-2xl" : "border-2 border-gray-200 rounded-2xl"} px-1 lg:px-5 py-5 lg:py-10 my-4`}
      >
        <div className="flex items-center lg:gap-x-2">
          <div>
            <Radio
          ...
          <div className="flex items-center gap-x-4">
            ...
            <div>
              ...
              <div className="mb-2 bg-sky-100 rounded-full inline-flex px-3 py-[2px] gap-x-1 items-center">
                ...
                <span className="text-[13px] text-center text-sky-700 mt-1">
                  Secure & instant payment
                </span>
              </div>
              <p className="text-[15px] mb-2 w-full max-w-[250px] text-gray-500">
                Pay securely usng any UPI app like Google Pay, PhonePe, Paytm,
                etc.
              </p>
              <div className="flex items-center gap-x-3">
